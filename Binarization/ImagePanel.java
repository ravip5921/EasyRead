...
public class ImagePanel extends JPanel {
    ...
    public void setBuffBounding(Image _img, Component[] listedComp) {
        img = _img;
        bimg = new BufferedImage(img.getWidth(), img.getHeight(), BufferedImage.TYPE_INT_RGB);
        ...
        for (int i = 0; i < listedComp.length - 1; i++) {
            if (listedComp[i] != null) {
                ...
                for (int j = listedComp[i].getMinX(); j < listedComp[i].getMaxX(); j++) {
                    bimg.setRGB(j, listedComp[i].getMinY(), 0xff00ff00);
                    // System.out.println("fa");
                    bimg.setRGB(j, listedComp[i].getMaxY(), 0xff00ff00);

                }

                for (int j = listedComp[i].getMinY(); j < listedComp[i].getMaxY(); j++) {
                    bimg.setRGB(listedComp[i].getMinX(), j, 0xff00ff00);
                    bimg.setRGB(listedComp[i].getMaxX(), j, 0xff00ff00);
