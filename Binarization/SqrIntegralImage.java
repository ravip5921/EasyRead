...
public class SqrIntegralImage extends IntegralImage {
    ...
    protected void constructIntegralImage() {
        ...
        pixel[0][0] = image.pixel[0][0] * image.pixel[0][0]; // first value of integral image is the initial pixel value
                                                             // squared
        ...
        for (int i = 1; i < image.getWidth(); i++)
            for (int j = 1; j < image.getHeight(); j++)
                pixel[i][j] = pixel[i - 1][j] + pixel[i][j - 1] + image.pixel[i][j] * image.pixel[i][j]
                        - pixel[i - 1][j - 1];
    }
}
