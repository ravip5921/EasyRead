...
public class ImageUtility {
    ...
    /*
     * public static void writeBinarizedImage(Image image, int textColor, int
     * bgColor, String filename)
     * {
     * 
     * }
     * 
    ...
    public static Image getImage(BufferedImage bimage) {
        // wrap the buffered-image by a new Image object to store the read image
        // the type of image is gray if buffered image is gary and rgb if buffered image
        // is rgb else null is returned
        if (bimage == null) {
        ...
        } else // (bimage.getType() == BufferedImage.TYPE_INT_RGB || bimage.getType() ==
         ...
         * {
         * System.out.
         * println("Cannot create Image object. Buffered image type not supported.");
         * return null;
         * }
         */
        return image;
    ...
    public static int findMaxPixelValue(Image img, int xs, int xe, int ys, int ye) {// most appropriate for grayscale
                                                                                    // image
                                                                                    // xs = starting bound for x coord,
                                                                                    // xe = ending bound for x
                                                                                    // coord(inclusive), similar for ys
                                                                                    // and ye
        int max = 0;
        for (int i = xs; i <= xe; i++)
            for (int j = ys; j <= ye; j++) {
    ...
    public static Image enhanceText(Image img, int imageMean, int imageSD) {// perform enhancement using pixel range
                                                                            // segmentation approach for garyscale image
    ...
    public static Image dilate(Image image, ImageWindow imageWindow) {
        if (image.getType() != Image.TYPE.BIN) {
            System.out.println("Doesn't support dilation for non-binary image");
            return null;
        }
    ...
    public static Image erode(Image image, ImageWindow imageWindow) {
        if (image.getType() != Image.TYPE.BIN) {
            System.out.println("Doesn't support erosion for non-binary image");
            return null;
        }
